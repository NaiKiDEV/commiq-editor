...
type ContainerLogsProps = {
  ...
  /** When provided shows a standalone header with back button. Omit when inside ContainerDetail tabs. */
  onBack?: () => void;
};

const MAX_LINES = 10_000;

export function ContainerLogs({ container, onBack }: ContainerLogsProps) {
  ...
  useEffect(() => {
    ...
    const unsub = window.electronAPI.docker.onLogChunk(streamId, (chunk) => {
    ...
    window.electronAPI.docker.logsStart(container.ID, streamId).catch(() => setStreaming(false));

    return () => {
      unsub();
      window.electronAPI.docker.logsStop(streamId);
      setStreaming(false);
    };
  }, [container.ID]);
  ...
  // Toolbar (shared between standalone and tab-embedded modes)
  const toolbar = (
    <div className="flex items-center gap-1 px-3 py-1.5 border-b border-border bg-background shrink-0">
      {streaming && (
      ...
      <div className="flex items-center gap-0.5 ml-auto">
        ...
        <Button
          ...
          title="Filter logs"
          className={cn(filter && 'text-primary')}
        >
          <Search className="size-3" />
        </Button>
        <Button variant="ghost" size="icon-xs" onClick={handleCopy} title="Copy logs">
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Standalone header (only when onBack is provided) */}
      {onBack !== undefined && (
        <div className="flex items-center gap-2 px-4 py-2.5 border-b border-border shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <h2 className="text-xs font-semibold truncate">{container.Names.replace(/^\//, '')}</h2>
            <p className="text-[10px] text-muted-foreground">{container.Image}</p>
          </div>
        </div>
      )}
      ...
      >
        ...
        {filteredLines.map(({ line, index }) => (
          <div
            key={index}
            className={cn('hover:bg-muted/30 px-1 -mx-1 rounded-sm', filter && 'bg-yellow-400/5')}
          >
      ...
      {paused && (
        <div className="px-4 py-1 bg-yellow-400/10 border-t border-yellow-400/20 text-center shrink-0">

...
function TextNodeBase({
  ...
  const handleClick = useCallback(
    (e: Konva.KonvaEventObject<MouseEvent>) => onClick(node.id, e),
    [node.id, onClick],
  );
  ...
  const handleDragStart = useCallback(
    (e: Konva.KonvaEventObject<DragEvent>) => onDragStart(node.id, e),
    [node.id, onDragStart],
  );
  const handleDragMove = useCallback(
    (e: Konva.KonvaEventObject<DragEvent>) => onDragMove(node.id, e),
    [node.id, onDragMove],
  );
  const handleDragEnd = useCallback(
    (e: Konva.KonvaEventObject<DragEvent>) => onDragEnd(node.id, e),
    [node.id, onDragEnd],
  );
  const handleContextMenu = useCallback(
    (e: Konva.KonvaEventObject<PointerEvent>) => onContextMenu(node.id, e),
    [node.id, onContextMenu],
  );

  const fontStyle =
    [node.italic ? "italic" : "", node.bold ? "bold" : ""]
      .filter(Boolean)
      .join(" ") || "normal";
...
export const TextNodeComponent = memo(
  TextNodeBase,
  (prev, next) =>

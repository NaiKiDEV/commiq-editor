...
export const ContextMenus = memo(function ContextMenus({
  ...
  return (
    <>
              ...
              {ALL_COLORS.map((color) => {
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      onStickyColorChange(contextMenu.stickyId, color)
                    }
                    title={
        ...
        >
          <div className="px-3 py-2 flex items-center gap-2 flex-wrap">
            {FRAME_COLORS.map((color) => {
              const currentColor = board?.frames.find(
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    onFrameColorChange(frameContextMenu.frameId, color)
                  }
                />
              );
            })}
      ...
      {connectionContextMenu && (
        ...
        >
          ...
          <button
            className="w-full px-3 py-1.5 text-sm text-red-400 hover:text-red-300 hover:bg-white/5 text-left"
            onClick={() =>
              onConnectionDelete(connectionContextMenu.connectionId)
            }
          >
            Delete
          </button>
      ...
      {textContextMenu &&
        (() => {
          const textNode = board?.texts.find(
                  ...
                  <div className="flex items-center gap-1.5 flex-wrap">
                    {TEXT_COLORS.map((color) => (
                      <button
                        ...
                        style={{ background: color }}
                        onClick={() =>
                          onTextColorChange(textContextMenu.textId, color)
                        }
                      />
                    ))}
                  </div>
                  ...
                  <div className="flex items-center gap-1 flex-wrap">
                    {TEXT_FONT_SIZES.map((size) => (
                      <button
                        ...
                        onClick={() =>
                          onTextSizeChange(textContextMenu.textId, size)
                        }
                      >
                        {size}
                      </button>
              ...
              {entries.map(([key, value], index) => (
                <div key={index} className="flex items-center gap-1.5 mb-1.5">
                  <input
                    ...
                    value={key}
                    onChange={(e) =>
                      onMetadataKeyChange(metadataEditor, index, e.target.value)
                    }
                  />
                  <input
                    className="flex-1 bg-white/5 border border-white/10 rounded px-2 py-1 text-xs text-white/80 outline-none focus:border-white/30"

...
const ProcessRow = memo(
  function ProcessRow({
    ...
    return (
      ...
      >
        <td
          className="px-4 py-1.5 font-medium max-w-[160px] truncate"
          title={entry.name}
        >
          {entry.name}
        </td>
        ...
        <td className="px-4 py-1.5 text-muted-foreground truncate max-w-[96px]">
          {entry.user || "—"}
        </td>
        <td className="px-4 py-1.5 font-mono">
          {entry.memoryMB.toFixed(1)} MB
        </td>
        <td className="px-4 py-1.5 font-mono text-muted-foreground">
          {entry.virtualMB.toFixed(1)} MB
        </td>
        {live && (
          <td className="px-4 py-1.5 font-mono">
        ...
        <td className="px-4 py-1.5 text-muted-foreground">
          {formatRelativeTime(entry.started)}
        </td>
        <td
...
export function ProcessMonitorPanel({
  panelId: _panelId,
}: {
  panelId: string;
}) {
    ...
    <div className="flex flex-col h-full bg-background">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-border flex-wrap">
        ...
        <Button
          ...
          title={
            live ? "Disable live CPU sampling" : "Enable live CPU sampling"
          }
        >
          Live
        </Button>
